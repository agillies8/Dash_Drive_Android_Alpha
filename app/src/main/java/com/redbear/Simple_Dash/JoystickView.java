...
public class JoystickView extends View implements Runnable {
        ...
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (onJoystickMoveListener != null)
                onJoystickMoveListener.onValueChanged(getAngle(), getPower(),
                        getDirection(), getThrottle(), getDirectionLR());
        }
        if (onJoystickMoveListener != null
    ...
    private double getThrottle() {
       double throttle = (centerY - yPosition) / joystickRadius *100;
       // Math.round(throttle);
   return  throttle;
    }

    private int getDirectionLR() {
        double directionLR = (xPosition - centerX) / joystickRadius *100;
        // Math.round(throttle);
        return (int) directionLR;
    }
                ...
                public void run() {
                    if (onJoystickMoveListener != null)
                        onJoystickMoveListener.onValueChanged(getAngle(),
                                getPower(), getDirection(), getThrottle(), getDirectionLR());
                }
            });
            try {
